    ...
    for i in sort([op(dir[i])]) do
        ...
        if signo[j] < 0 then
            if abs(i) < .0001 then grfnud := grfnud union {
                perspectiva(torsorPerspnN(Nud[N, 1] + 2.5*esc2*esc,
                0, esc, 0, m[j], 1, Y = Nud[N, 2]), Y = Nud[N, 2])}
            fi;
            if abs(i - 3.141592654) < .0001 then grfnud := grfnud
                union {perspectiva(torsorPerspnN(
                Nud[N, 1] - 2.5*esc2*esc, 0, esc, 0, m[j], 1,
                Y = Nud[N, 2]), Y = Nud[N, 2])}
            fi;
            if abs(i + 1.570796327) < .0001 then grfnud := grfnud
                union {perspectiva(torsorPespnN(
                Nud[N, 2] - 2.5*esc2*esc, 0, esc, 0, m[j], 1,
                X = Nud[N, 1]), X = Nud[N, 1])}
            fi;
            if abs(i - 1.570796327) < .0001 then grfnud := grfnud
                union {perspectiva(torsorPerspnN(
        ...
        if 0 < signo[j] then
            if abs(i) < .0001 then grfnud := grfnud union {
                perspectiva(torsorPerspN(Nud[N, 1] + 2.5*esc2*esc, 0,
                esc, 0, m[j], 1, Y = Nud[N, 2]), Y = Nud[N, 2])}
            fi;
            if abs(i - 3.141592654) < .0001 then grfnud := grfnud
                union {perspectiva(torsorPerspN(
                Nud[N, 1] - 2.5*esc2*esc, 0, esc, 0, m[j], 1,
                Y = Nud[N, 2]), Y = Nud[N, 2])}
            fi;
            if abs(i + 1.570796327) < .0001 then grfnud := grfnud
