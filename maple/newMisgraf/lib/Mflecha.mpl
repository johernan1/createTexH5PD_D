...
global MiVERDE, MiCELESTE;
    ...
    poly1 := [[0, 0], [1, 1], [0, 1]];
    poly1 := trasf(poly1, x, y, esc, alpha, alpha);
    poly2 := [[.3, 1], [.3, 2.5], [0, 2.5], [0, 1]];
    poly2 := trasf(poly2, x, y, esc, alpha);
    poly3 := [[0, 0], [1, 1], [.3, 1], [.3, 2.5], [0, 2.5], [0, 0]];
    poly3 := trasf(poly3, x, y, esc, alpha);
    PLOT(POLYGONS(poly1, poly2, STYLE(PATCHNOGRID), COLOUR(MiVERDE)),
        CURVES(poly3))
end
